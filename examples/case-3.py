...
DEFAULT_OUTPUT_PATH = VALIDATION_RESULT_DIR / "case-3-result.json"
FLOW_PATTERNS = ["co-current", "counter-current"]


def load_reference_content() -> str:
    module_path = Path(__file__).resolve().parent / "reference.py"
    spec = importlib.util.spec_from_file_location("case_3_reference", module_path)
...
def build_request(reference_content: str, flow_pattern: str) -> dict[str, Any]:
    solver_options = {"rtol": 1e-6, "atol": 1e-9}
    if flow_pattern == "counter-current":
        solver_options = {
    ...
    return {
        ...
        "components": [
            {"name": "oxygen", "formula": "O2", "state": "g", "mole_fraction": 0.205},
            {"name": "nitrogen", "formula": "N2", "state": "g", "mole_fraction": 0.795},
        ],
        "feed_volumetric_flow": load_case_flowrate(),
        "feed_inlet_temperature": {"value": 296.15, "unit": "K"},
        "feed_pressure": {"value": 690, "unit": "kPa"},
        "permeate_inlet_temperature": {"value": 296.15, "unit": "K"},
        "permeate_pressure": {"value": 100, "unit": "kPa"},
        "gas_transport_coefficients": {
            "O2-g": {"value": 9.30, "unit": "GPU"},
            "N2-g": {"value": 1.80, "unit": "GPU"},
        },
        "module_geometry": {
            "number_of_fibers": {"value": 368, "unit": ""},
            "fiber_outer_diameter": {"value": 0.016, "unit": "cm"},
            "fiber_inner_diameter": {"value": 0.008, "unit": "cm"},
            "fiber_length": {"value": 25, "unit": "cm"},
            "module_diameter": {"value": 0.95, "unit": "cm"},
        },
...
def run_case(flow_patterns: list[str]) -> dict[str, Any]:
    reference_content = load_reference_content()
    simulations: dict[str, Any] = {}
    for flow_pattern in flow_patterns:
        request = GasHFMSimulationRequest(**build_request(reference_content, flow_pattern))
        simulations[flow_pattern] = simulate_gas_hfm(request)

    return {
        "case": CASE_ID,
        "source": "examples/case-3.md",
        "assumptions": {
            "feed_volumetric_flow_source": str(FLOWRATES_PATH.relative_to(PROJECT_DIR)),
            "number_of_elements": "50 documented in case-3.md; solver uses adaptive points or BVP mesh_points",
            "packing_fraction": "documented in case-3.md but not accepted by the current module_geometry schema",
        },
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run validation simulation for examples/case-3.md.")
    parser.add_argument(
        "--flow-pattern",
        choices=FLOW_PATTERNS,
        action="append",
        help="Flow pattern to run. Repeat to run both. Defaults to both case flow patterns.",
    )
    parser.add_argument(
        "--output",
        type=Path,
        default=DEFAULT_OUTPUT_PATH,
        help="Path for the complete JSON result. Defaults to validation result/case-3-result.json.",
    )
    parser.add_argument(
        "--print-full",
        action="store_true",
...
def main() -> None:
    args = parse_args()
    flow_patterns = args.flow_pattern or FLOW_PATTERNS
    result = run_case(flow_patterns)
    ...
    summary = {
        ...
        "simulations": {
            ...
            for flow_pattern, simulation in result["simulations"].items()
        },
    }
